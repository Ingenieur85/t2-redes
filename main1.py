...
def receive_message(sock, timeout=5):
    ...
    try:
        ...
        print(f"Received message: {data.decode()}")  # Debug print
        return data.decode()
    except socket.timeout:
        print("Receive timeout occurred")  # Debug print
        return None
...
def initialize_game(sock, player_num, next_address):
    print(f"Player {player_num} initializing...")  # Debug print
    if player_num == 1:
        print("Player 1 waiting for INIT message from Player 4...")
        while True:
            ...
            if msg == "INIT 4":
                ...
                print("Timeout while waiting for INIT message. Retrying...")
    else:
        print(f"Player {player_num} waiting for INIT message from Player {player_num - 1}...")
        while True:
            msg = receive_message(sock)
            if msg == f"INIT {player_num - 1}":
                print(f"Received INIT from Player {player_num - 1}. Sending ready signal...")
...
def main(player_num):
    ...
    print(f"Next player address: {next_address}")  # Debug print

    if not initialize_game(sock, player_num, next_address):
